...
function initGameContainer() {
  ...
  if (!c) {
    ...
    c.id = "game-container";
    c.className =
      "absolute inset-0 flex flex-col items-center justify-center px-4 bg-black bg-opacity-75";
...
async function renderQuestion(container) {
  ...
  // Fetch track
  const { title, artist, preview, era } = await getRandomTrackFromRandomEra();
  ...
  const options = [lastAnswer, ...extras.slice(0, 3)].sort(() => 0.5 - Math.random());

  options.forEach((opt) => {
    const btn = document.createElement("button");
    btn.className =
      "w-full max-w-md py-4 my-2 bg-purple-900 bg-opacity-50 hover:bg-opacity-80 rounded-lg text-xl font-semibold text-white transition";
    btn.textContent = opt;
    btn.onclick = () => {
